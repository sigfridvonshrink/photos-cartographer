...
def setup_workspace(tmp_path: Path):
    ...
    (ws / "4-photos-by-date" / "20230101_120000.jpg").write_text("already_date")
...
def test_prep_idempotency_and_by_dest_accounting(mock_meta, tmp_path):
    ...
    assert any(f.startswith("4-photos-by-date/20230101_120000-001") for f in all_files.keys())
...
def test_handoff_manifest_generated(mock_meta, tmp_path):
    ...
    manifest_path = ws / ".photos-ingest" / "photos-1-prep-handoff.json"
...
def test_stale_cache_upsert_fails(mock_meta, tmp_path):
    ...
    assert not (ws / ".photos-ingest" / "photos-1-prep-handoff.json").exists()
