...
When the user moves a file from by-date into 5-photos-by-dest (or re-sorts one
...
def _seed_one(ws, content=b"AAAA"):
    """Run prep once so a 0-source file is organized into 4-photos-by-date and cached."""
    (ws / "0-source" / "a.jpg").write_bytes(content)
    prep.CONFIG["jobs"] = 1
    plan = _plan(ws)
...
def test_by_date_to_by_dest_move_is_recognized(tmp_path, monkeypatch):
    ...
    dest_dir = ws / "5-photos-by-dest" / "Belgium"
    ...
    assert all("5-photos-by-dest" not in (op.source or "")
               and "5-photos-by-dest" not in (op.destination or "")
               for op in _media_ops(plan2))

    prep.PlanExecutor(str(ws)).execute(plan2)
    rows = prep.WorkspaceCache(str(ws)).get_all_files()
    new_rel = "5-photos-by-dest/Belgium/" + os.path.basename(organized)
...
def test_re_sort_between_destinations_is_recognized(tmp_path, monkeypatch):
    ...
    (ws / "5-photos-by-dest" / "Brussels").mkdir(parents=True)
    os.rename(organized, ws / "5-photos-by-dest" / "Brussels" / name)
    prep.PlanExecutor(str(ws)).execute(_plan(ws))

    # Now re-sort Brussels -> Bruges (§10.2).
    (ws / "5-photos-by-dest" / "Bruges").mkdir(parents=True)
    os.rename(ws / "5-photos-by-dest" / "Brussels" / name,
              ws / "5-photos-by-dest" / "Bruges" / name)
    hash_calls.clear()
    plan3 = _plan(ws)
    ...
    rows = prep.WorkspaceCache(str(ws)).get_all_files()
    assert "5-photos-by-dest/Bruges/" + name in rows
    assert "5-photos-by-dest/Brussels/" + name not in rows
    handoff = json.load(open(utils.handoff_path(str(ws))))
    assert any(df["path"] == "5-photos-by-dest/Bruges" for df in handoff["destination_folders"])
...
def test_ambiguous_match_is_rescanned_not_carried(tmp_path, monkeypatch):
    ...
    (ws / "5-photos-by-dest" / "A").mkdir(parents=True)
    (ws / "5-photos-by-dest" / "B").mkdir(parents=True)
    a = ws / "5-photos-by-dest" / "A" / name
    b = ws / "5-photos-by-dest" / "B" / name
...
def test_content_change_is_rescanned_not_carried(tmp_path, monkeypatch):
    ...
    dest_dir = ws / "5-photos-by-dest" / "Trip"
    dest_dir.mkdir(parents=True)
    new_abs = dest_dir / os.path.basename(organized)
    os.rename(organized, new_abs)
