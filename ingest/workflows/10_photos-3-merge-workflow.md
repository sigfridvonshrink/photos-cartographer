...
8. **Final status** — `success`, `partial`, `failed`, or `rejected` (a pre-mutation abort — e.g. a required ZFS snapshot that could not be taken, Section 10.3 step 3 — that mutated nothing but records the snapshot/abort detail for audit, mirroring calibration's rejected execution summary; a *stale-plan* rejection writes no summary at all, Section 5 invariant 9).
