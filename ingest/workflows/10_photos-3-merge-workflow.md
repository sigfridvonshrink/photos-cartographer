...
10. **mark the workspace terminal (sealed)** on overall success — record the durable terminal/sealed marker (merge run id, `library_root`) as a **separate `photos-00-sealed.json` file alongside the workspace guard** (shared contract Sections 5.1, 13.7), after which prep and calibration refuse to process new media in this workspace (prune-quarantine excepted, shared contract Section 13.7 item 2);
...
By-dest changed since finalize (photo added/edited + re-prep, but no re-calibrate)
  -> current handoff content_fingerprint != the one photos-23 recorded -> precondition 1a blocker
  -> nothing moved; "re-calibrate and re-finalize before merging" (avoids merging a
     present-but-uncalibrated photo). A no-op re-prep (run-metadata only) does NOT trip this.
