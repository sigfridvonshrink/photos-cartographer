...
7. **Final status** — `success` (clean), `partial` (some operations applied or already-satisfied, with failures/blockers/mismatches outstanding; a non-empty fingerprint-mismatch list always forces `partial`, item 8), `failed` (execution ran but nothing was applied or already-satisfied — total failure), or `rejected` (a pre-mutation abort — e.g. a required snapshot that could not be taken, Section 29 step 6 — that mutated nothing but records the abort for audit; a stale-plan rejection writes no summary at all);
