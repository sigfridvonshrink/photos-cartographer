...
Each phase that mutates can take an optional pre-mutation snapshot before applying any planned mutation, using the same mechanism so the phases behave identically and cannot diverge (prep and geotag snapshot the workspace tree; merge optionally snapshots the library volume — closing note below). **Snapshots are an optional extra safety layer, not a requirement.** The pipeline operates fully and safely without them: correctness and recoverability rest on the journal, recoverable quarantine, no-clobber operations, and filesystem-as-truth reconciliation (prep Section 14.4; geotag Section 29.1; merge Section 8). Snapshots add a clean-slate rollback path on top of those for operators whose filesystem supports it; they are not the basis of the safety model.
...
1. **Seeded by prep.** On a prep run, if `photos-00-config.json` is absent, prep creates it from a default template (a deployment may supply its own seed defaults; *how* it does so is a deployment concern, not part of this contract). If it is present, prep uses it as-is. The seed defaults are therefore only a **template for seeding a fresh workspace**; once the file exists, the workspace copy is authoritative and governs all processing in that workspace, regardless of what the defaults later become.
2. **Prep is the sole writer of config in the data pipeline; the user hand-edits.** Like the handoff, the config file is written by prep alone in the prep→geotag→merge data flow (prep seeds it once). (The SQLite DB is shared-write across phases — prep owns its cache/identity content, geotag its derived regions (resolved-UTC cache and manual-GPS pre-state ledger), and merge its library-file fingerprint cache, all disjoint, Section 13.4 — but the config file, like the handoff, has a single writer in that flow: prep.) The one narrow exception is the **one-time `init-library` setup command** (merge spec `photos-3-merge-workflow.md` Section 4), which, when run from inside a workspace with an explicit library path, may write the single `library_root` key — and only that key — into config, as an operator convenience for recording which library this workspace merges into. That setup write is not part of the merge *data path*: merge's `plan`/`dry-run`/`execute` are strictly **read-only** with respect to config, exactly like geotag, which never writes it. To change any other configuration, the user edits the JSON by hand — it is an authored input in the same sense as the decision fields (Section 12).
...
So two things are derived from the file: (a) field-scoped fingerprints (filename-format, camera-time/timezone policy, GPX thresholds, camera-group-key version, snapshot policy, the **folder-name set**, the **media-extension classification lists**, …) that drive precise staleness; and (b) one whole-file hash for integrity/provenance. The folder-set and media-extension fingerprints drive the organization-shaped stages: a change to either restales the geotag executable plan and the merge plan (which locate by-dest by name and classify the tree), because renaming a managed folder or moving an extension between media and `other` changes which files exist where. (Prep re-plans from the filesystem each run and guards execute with the whole-file hash, so it needs no separate folder/extension field-fingerprint of its own.)
...
3. camera-group **classification**: `camera_time_and_timezone_policy.device_groups` (`phones`, `fixed_clock_cameras`) (Section 6) — the only camera-grouping value that lives in config. The derivation *version*, `CAMERA_GROUP_KEY_VERSION`, is **not** a config field: it is a code constant that versions how `camera_group_key` is computed and participates in the dependency fingerprints (Section 6, Section 9), so a change to the derivation is detectable. The user classifies device groups in config; the user never sets the key-derivation version;
...
6. camera-time/timezone policy flags, e.g. `single_anchor_auto_apply`, `multi_anchor_auto_apply` (consumed by geotag);
...
8. the **managed folder names**: `folders`, a role→name map for the numbered managed folders (Section 5, prep `photos-1-prep-workflow.md` Section 3). The *names* are config (the user may rename `0-sources`…`6-photos-by-dest`); the *roles* and the 0–6 structure invariant are **not** config — they are pipeline logic (inbox, read-only by-dest, dedup-retention order, photo/video band membership) referenced by role, never by literal name. Same boundary as item 3: the user sets names, never the roles;
9. the **media-extension classification lists**: `media_extensions`, with the keys `image` / `raw` / `video` mapping to lists of canonical (lowercase, no leading dot) extensions (prep `photos-1-prep-workflow.md` Section 6.1). The *lists* are config; the *class vocabulary* (`image`/`raw`/`video`, with `other` as the residual for any unlisted extension) is code. An extension maps to exactly one class.
...
.photos-ingest/
  ...
  photos-00-plan-memo.db           prep-phase scratch: plan-time decode memo (disposable, NOT part of the record, prep photos-1-prep-workflow.md Section 10.3)
  ...
  photos-22-gps-drift-validation.json  geotag-phase artifact: GPS-drift validation gate (§22a)
  ...
  photos-26-complete-log.json      geotag-phase artifact: full transformation log (prep+geotag, at finalize, Section 13.3)
  photos-26-geotag-ingest.db    geotag-phase artifact: DB backup snapshot, end of geotag (Section 13.4a)
  photos-31-merge-summary.json     merge-phase artifact: library-merge summary (Section 13.5)
  photos-35-merge-log.json         merge-phase artifact: full transformation log (prep+geotag+merge, Section 13.3)
...
2X  geotag-phase artifacts (currently: 21–25; 25 = complete log + geotag DB snapshot)
3X  merge-phase artifacts        (currently: 31 = merge summary, 35 = merge log + merge DB snapshot)
```

That is the whole meaning of the scheme: read the first digit to know the producing phase. The second digit is just a sequence number within the phase and carries no further significance — absent numbers (there is no `photos-07`, no `photos-12`–`14`, etc.) are not reserved, not headroom, and not missing; they simply don't exist. Do not infer anything from a gap. A number may be shared by sibling end-of-phase records that differ only by extension — `photos-15-prep-log.json` and `photos-15-prep-ingest.db` are both "the end-of-prep record," one the human-readable log and one the DB image; likewise `photos-26-complete-log.json` and `photos-26-geotag-ingest.db`. The full filename (number + name + extension) is the identifier, not the number alone. The shorthand `photos-0X-*` (and `1X`/`2X`/`3X`) refers to a band of numbered artifacts collectively. (Note: the artifact-number bands `0X`–`3X` are unrelated to the workspace's numbered media folders `0-sources`…`6-photos-by-dest`; the folder `1-strays/` is a media-tree folder, not an artifact band.)
...
Both phases identify a camera/device by a single derived key so grouping is computed once (by prep) and reused (by geotag), never reinvented.
...
Both phases name files from a timestamp using a **single shared, config-driven format**. There is one format key for the whole pipeline, not one per phase, so the provisional names prep assigns and the final names geotag assigns are guaranteed to share the same textual shape and can never drift apart.
...
The format is never hard-coded in either phase; both read this key, and the key participates in the dependency cascade (Section 9). Changing it makes any artifact whose names depend on it stale — in prep, the by-date organization; in geotag, the executable rename plan.

> The geotag specification historically referred to this key as `geotag_filename_timestamp_format`. It is the **same** key as `filename_timestamp_format` defined here; the phase-neutral name is authoritative because the key is shared, and any remaining phase-specific name is an alias for it.
...
- **Prep and geotag** take the **first free** name: the bare `<timestamp>.ext` when available, otherwise the lowest free `-NNN`. Bare-first is what makes an uncorrected file's provisional (prep) and final (geotag) names coincide, so geotag plans no needless rename (Section 7.3); the two phases use the identical rule so they never disagree on a name.
...
Suffix allocation rules are specified per phase where the renames are planned (prep Section 8 / Section 7.3; geotag Section 27; merge spec Section 7).

### 7.3 Provisional (prep) vs. final (geotag) timestamps
...
2. **Geotag** recomputes the timestamp from *corrected, destination-local civil time* (resolved UTC → destination civil timezone → local naive datetime → this format) and rewrites the name to its **final** value.

Because the shape is identical, a file's name changes between phases only when its timestamp is genuinely corrected; an uncorrected file's provisional and final names coincide, so geotag plans no needless rename for it.

### 7.4 Both phases use the same format; non-conforming names are dumps

Both prep and geotag read the same `filename_timestamp_format` and emit the same name shape, so a file's provisional (prep) and final (geotag) names differ only when the timestamp is actually corrected (Section 7.3). There is no format divergence to reconcile.
...
Both phases enforce the same anti-stale rule, stated in full in each phase spec (prep Section 5; geotag Sections 3–6):
...
4. GEOTAG       convergent rerun loop (cal 2.1):
   (loop)          run -> inspect blockers -> edit human-decision fields in the
                   photos-2X JSON -> rerun -> ... until photos-24-executable-plan.json is ready.
...
6. (optional)      add more media later: drop a new dump into 0-sources, then
   ADD MORE        re-run the loop from PREP (step 1) — prep organizes the new
                   ...
                   the move, then RE-RUN GEOTAG/EXECUTE (steps 1-5). Allowed even
                   after a prior successful geotag.

7. FINALIZE        explicit command, once "this dump is done" (Section 13.1):
                   assemble the archival package and generate the full
                   transformation log photos-26-complete-log.json.
                   Required before merge; kept separate from merge because
                   geotag is freely re-run and re-finalizing each pass
                   would be wasteful.
...
8. MERGE           explicit command (photos-3-merge): requires a finalized
                   ...
                   log photos-35-merge-log.json (copied forward from photos-26,
                   ...
                   dumps/prep/geotag in it (Section 13.7).
   ...
   (after merge)   (new control dir + new SQLite DB) and run from step 0. The
                   ...
                   ALL scripts (prep, geotag, merge) HARD-STOP on it and
...
9. DEVELOPMENT     the jpg/tif breakout. A later processing phase, applied in
   (later phase)   the library after merge. Starting it inside the workspace
                   before geotag is settled is a hard-stop. [cal 7.1]
```

### 10.1 Geotag is freely re-runnable, not one-shot

Geotag is not a terminal, run-once phase. It may be re-run any number of times — including after a prior successful execution — to absorb media added later, **up until the workspace is merged**. Merge seals the workspace (Section 13.7), and a sealed workspace accepts no further prep or geotag; the freedom to re-run geotag exists across the prep→geotag cycles of a single workspace's life, and ends when that workspace's batch is merged into the library. (More media after a merge means a fresh workspace, Section 13.7 / 10.4.) The re-runnability before sealing is safe because:

1. human decisions (timezones, accepted anchors, manual offsets, manual GPS) live in the `photos-2X` geotag JSON and are preserved across regeneration where their logical target is unchanged (cal Section 9);
2. GPS that geotag applied is recognizable on rerun (e.g. via the `GPSProcessingMethod` marker), so already-placed files are no-ops, not re-writes;
...
What bounds the freedom is a small, fixed set of gating preconditions, not a "has geotag run before?" flag. Geotag may proceed whenever, and only when:
...
Condition (d) is why the re-prep of step 3 is mandatory every time new photos enter by-dest, even post-geotag: geotag depends on the prep handoff as a hashed artifact (cal Section 4), and files prep has not yet recognized are not in it. New photos never reach geotag "behind prep's back."
...
The operator may run several prep → sort cycles before ever geotagging (import a batch, sort it, import more, sort more). Geotag cares only that the *current* state satisfies the gating preconditions above; it does not require that prep and sort happened exactly once. The loop is a steady state to return to, not a one-way pipeline.

The entire loop runs under the single workspace lock (Section 2): only one pipeline process — prep or geotag — touches the workspace at a time.
...
1. **Position.** Prep → sort → re-prep → geotag → execute → finalize → **merge → sealed**. Merge consumes a geotagged, finalized by-dest and **moves** it into the library (removing merged sources from by-dest); it is the last pipeline action against a given dump's workspace, after which the workspace is sealed (Section 13.7).
2. **Preconditions.** Merge requires that geotag ended successfully and the workspace was finalized (the archival package and `photos-26-complete-log.json` exist), so what merge moves into the library is the corrected, named, fully-recorded set — never a half-geotagged one. Because finalize is a point in time and by-dest can change after it, merge does not merely trust that finalize was last: it re-validates that the finalized plan is **current** with the present by-dest (recomputing `photos-11-handoff.json`'s content fingerprint against the value `photos-24-executable-plan.json` recorded — so a real by-dest change is caught while a run-only re-prep is not), and hard-stops directing the operator to re-run geotag and re-finalize if a by-dest photo postdates finalize — so "never a half-geotagged one" is mechanically enforced, not assumed. The detailed preconditions are in the merge spec (merge spec Section 3, precondition 1a).
3. **Re-seals the archive automatically.** Unlike geotag's finalize (a deliberate separate step, kept manual because geotag is freely re-run), merge re-seals the archival package automatically on success — it has added to the durable record (its own `photos-35-merge-log.json` copied forward from `photos-26` and extended, the end-of-merge DB snapshot, the merge summary), so it bundles the now-current archive without a manual re-finalize (Section 13.6).
4. **Terminal: the workspace is sealed, not recycled.** A successfully merged workspace is marked terminal and **every media-mutating script refuses to run on it** (Section 13.7) — even if by-dest looks empty, the workspace is not a clean slate (its DB, ledgers, caches, snapshots, and sealed archive persist). To add more media, the operator starts a **fresh workspace** and runs from the start; the shared permanent library accepts merges from many workspaces (the library-side lock, Section 15.2). Sealed means sealed: prep, geotag, **and merge** all hard-stop and touch nothing — there is no confirming re-merge and no recovery utility (Section 13.7). The sole exception is prep's `prune-quarantine`, which is still allowed because it only reclaims recoverable quarantine copies and touches no controlled content (Section 13.7 item 2).
5. **Development is downstream of merge.** The jpg/tif development breakout (cal Section 7.1) is a library-side processing phase that runs *after* a batch is merged, never inside the workspace before geotag settles.
...
1. **Reuse, don't recompute.** Work already done and still valid is reused, not repeated — prep reuses cached fingerprints and metadata for files whose size/mtime (and move-aware identity) are unchanged; geotag reuses resolved UTC and preserves human decisions whose logical target is unchanged. Expensive operations (fingerprinting, metadata extraction, GPX parsing) run only for genuinely new or changed inputs.
...
The phase specs implement this via geotag Sections 9 and 20 (authored, preserved decision fields) and the journal/fingerprint discipline of prep Section 14.3 and geotag Sections 4–6 and 29; merge adds no human-decision fields but its placement is the same deterministic-rule-on-recorded-inputs kind, recorded the same way (`photos-3-merge-workflow.md` Sections 8–9). This section is the single statement of the principle they share.
...
```text
                     ...
                     (as if geotag and merge did not exist)
end of GEOTAG  -> that record, extended through geotag
...
1. **Each phase brings the record up to date through its own work.** The per-photo **transformation log** is the human-readable spine, realized as one file per phase in the artifact-numbering scheme (Section 5): prep writes `photos-15-prep-log.json` (prep `photos-1-prep-workflow.md` Section 16.1); geotag's finalize produces `photos-26-complete-log.json` by **copying the prep log forward under its own phase name and extending the copy** (Section 13.3); merge produces `photos-35-merge-log.json` by copying the geotag log forward and extending it (Section 13.5). Each step is **additive and owned**: a phase writes only its own phase-named log and never rewrites or invalidates an earlier phase's file (Section 13.0a) — it copies the predecessor forward and appends. (Geotag does not re-derive prep's history; it consumes `photos-15-prep-log.json` and the prep journals as the prep portion of each journey.)
2. **The SQLite database** (Section 13.4) is a live durable record updated by each phase, complete-as-of-now at every phase boundary; and at the end of each phase a **point-in-time backup snapshot** of it is captured and retained (`photos-15-prep-ingest.db`, `photos-26-geotag-ingest.db`, `photos-35-merge-ingest.db`, Section 13.4a), so a full DB image is preserved as of each phase, not just the last.
3. **The per-phase job artifacts** — prep's handoff/summary, geotag's decision/plan/execution-summary artifacts, merge's summary — each describe what their phase did and remain on disk.
4. **No later phase is a prerequisite for an earlier phase's record being whole.** A workspace prepped but never geotagged has a complete prep audit record; one geotagged and finalized but never merged has a complete prep+geotag record; merge only *adds* the library destination. An absent later phase reads as "not advanced past phase N," never as "incomplete record."

The **archival package** (Sections 13.1–13.2) is the bundling of this record for retention. It is conventionally assembled at the finalize step (after geotag), because that is when a dump is usually "done"; but the *guarantee* that a complete audit record exists is not tied to finalize — it holds at every phase boundary. The subsections below specify the package and the cumulative log; Section 13.5 specifies merge's additive extension.
...
```text
A phase writes only artifacts in its own number band (prep 1X, geotag 2X, merge 3X).
...
This is why the transformation log is three separate files rather than one mutated in place (Section 13.3): geotag does not edit prep's `photos-15-prep-log.json` — it copies it to `photos-26-complete-log.json` and extends that; merge does not edit `photos-26-complete-log.json` — it copies it to `photos-35-merge-log.json` and extends that. The same ownership holds for every artifact:
...
2. **Geotag (2X)** owns and writes `photos-21`–`photos-26` (including `photos-26-complete-log.json` and `photos-26-geotag-ingest.db`). It reads prep's 1X artifacts read-only and never modifies them. (It writes only its own disjoint live-DB regions, Section 13.4.)
3. **Merge (3X)** owns and writes `photos-31-merge-summary.json`, `photos-35-merge-log.json`, `photos-35-merge-ingest.db`. It reads geotag's and prep's artifacts read-only and never modifies them. (It writes only its own disjoint live-DB regions, Section 13.4.)

Re-running a phase may legitimately refresh **its own** artifacts (a re-prep updates `photos-15-*`; geotag reruns update `photos-2X-*`); it still never touches another phase's. The benefit is that each phase's outputs are a stable, attributable record that a later phase cannot silently alter — combined with the per-phase completeness guarantee above, every phase's artifacts are both complete and tamper-evident with respect to later phases. (The archival package bundles copies of all phases' artifacts, Section 13.2; bundling is a read, not a write, so it does not violate ownership.)

### 13.1 When the archival package is produced

The package is produced by an **explicit finalize/archive command**, not automatically at the end of each geotag. Because geotag is freely re-runnable (Section 10.1) — the user may geotag, add a dump, and geotag again — "this dump is done" is a human judgement, so package assembly is a deliberate, separate step the user invokes when finished with a workspace. The command runs under the same workspace lock as every other operation (Section 2) and is non-destructive: it reads and bundles, it does not mutate the workspace or the library. (Finalize bundles a record that already exists at the geotag phase boundary, Section 13.0; it consolidates and seals it, it does not create the audit record from nothing.)
...
2. **All current decision/plan JSON artifacts** — the prep handoff (`photos-11-handoff.json`); the numbered geotag artifacts `photos-21-time-decisions.json`, `photos-22-gps-drift-validation.json`, `photos-23-gps-decisions.json`, `photos-24-executable-plan.json`, `photos-25-execution-summary.json`; and, if merge has run, the merge summary (`photos-31-merge-summary.json`) — exactly as written (same bytes the pipeline validated against, Section 12 part 2).
3. **The SQLite database and its per-phase backup snapshots** — the live `photos-00-ingest.db` (Section 13.4) plus every per-phase backup snapshot present (`photos-15-prep-ingest.db`, `photos-26-geotag-ingest.db`, and, if merge has run, `photos-35-merge-ingest.db`, Section 13.4a): the DB is part of the durable record, not throwaway scratch, and is archived with the artifacts, together with a full point-in-time image as of each phase boundary.
...
### 13.3 The transformation log (`photos-26-complete-log.json`)

The transformation log answers, for any photo, "what happened to this file and why?" — in one place. It is a **derived, consolidated view** stitched from records that already exist: prep's execution journal and handoff, geotag's decision artifacts and execution journal, and merge's summary. It does not introduce new authority; it fuses the phases' existing records into a per-photo story, and it is **maintained cumulatively** — each phase brings it up to date through its own work (Section 13.0), so at every phase boundary there is a complete human-readable log of everything done so far.
...
```json
{
  ...
  "journey": [
    ...
    { "phase": "geotag", "action": "clock_offset_applied",
    ...
    { "phase": "geotag", "action": "resolved_utc", "value": "2024-07-03T12:12:21Z" },
    { "phase": "geotag", "action": "timezone_resolved",
      "value": "Europe/Brussels", "because": "destinations.Belgium/Brussels.user_decision" },
    { "phase": "geotag", "action": "gps_written",
      "lat": 50.8467, "lon": 4.3525, "method": "gpx_segment_interpolation", "gps_processing_method": "…" },
    { "phase": "geotag", "action": "final_rename", "to": "2024-07-03--14-12-21.arw" },
...
photos-26-geotag-ingest.db   DB image as of the end of geotag (geotag finalize)
...
3. **Captured at the same gate as the phase's other end-of-phase artifacts.** Prep captures `photos-15-prep-ingest.db` when it writes the handoff and prep log (prep Section 14.3 / 16.1); geotag captures `photos-26-geotag-ingest.db` at finalize alongside `photos-26-complete-log.json` (geotag Section 31); merge captures `photos-35-merge-ingest.db` when it writes the merge summary (merge Section 9 / 10.3). A phase that does not complete successfully writes no new snapshot, leaving the prior one intact.
...
2. **Every *mutating media* script refuses a sealed workspace — sealed means sealed.** Prep, geotag, **and merge** each check for the terminal marker at startup (right after taking the lock and verifying the guard, before any scan or plan). If it is present, the script **hard-stops immediately, mutating nothing and touching nothing** — there is no confirming re-run, no sweep, and no recovery utility. A sealed workspace's durable outputs may still be *read* (open the archive, inspect the DB), but no script ever organizes, geotags, or merges it again.
...
This makes "workspace" genuinely transient and single-batch: prep → … → geotag → finalize → merge → **sealed**. The durable outputs (the library and the archival package) live on; the workspace does not invite reuse.
...
3. **Timezones**: any IANA timezone string a user enters (e.g. a destination timezone decision, geotag Section 18) must be a real, resolvable zone, not an arbitrary string.
...
4. **Preserve, don't delete, an invalid authored value.** Consistent with geotag Section 9, an invalid user decision is preserved and flagged as requiring correction, not erased; the user retains what they wrote so they can see and fix it.

---

## 15. Execute-time no-clobber and filesystem atomicity

Both the safety model's "no operation overwrites existing content" promise and the resume guarantees depend on filesystem operations being **defensive at the moment they execute**, not merely planned to be safe. This section states the shared rule; prep Section 14.3 and 14.4, geotag Sections 27 and 29.1a, and the merge spec implement it.

1. **No-clobber is re-checked at execute time, not assumed from the plan.** A plan is computed against a snapshot of filesystem state and may be validated against current state (the dependency cascade, Section 9), but the executor must **still verify, immediately before each individual filesystem mutation, that the specific target does not already exist** (case-insensitively where the filesystem is case-insensitive). The planner's collision analysis (prep Section 8 / geotag Section 27, which treat every on-disk and planned name as permanently occupied) is the first line of defence; the execute-time check is the second, independent one. Execution must never write or rename onto an existing path on the strength of the plan alone — if the target unexpectedly exists at execute time, the operation is a blocker (or, where the operation class permits and the plan recorded a safe alternative allocation, it is re-checked against that), never a clobber. This applies to every content-bearing operation: prep moves/renames/quarantine, geotag renames, and merge placement into the library.
2. **Operations are atomic where the filesystem allows.** A move, rename, or placement should be a single atomic filesystem operation so an interruption leaves either the pre-state or the post-state, never a partial result. A metadata write that rewrites a file in place is performed atomically — write to a temporary file and atomically rename it into the final name, or use the tool's safe-write mode — so a crash mid-write leaves the intact original or the fully-written file, never a corrupt intermediate (geotag Section 29.1a). Atomicity and the execute-time no-clobber check compose: the atomic rename that finalizes an operation is itself a no-clobber rename onto a target just verified free.
3. **Cross-filesystem placement stays atomic-equivalent; a move removes the source last.** Where an operation crosses filesystem boundaries (notably merge **moving** files into a library that may be on a different volume, so a same-inode `rename` is unavailable), the executor must preserve the same guarantee by an equivalent technique: copy to a temporary name on the destination filesystem, verify, then atomically rename into the final (just-verified-free) target, and treat a crash as leaving either no destination file or the complete one — never a half-copied file presented under the final name. For a **move** (merge), the source is removed only **after** the destination copy is verified in place, as the last journaled step — so a crash leaves the file in the source, or in both source and destination (resume completes the source removal), but never lost and never half-present (merge spec Section 11). The cross-filesystem temporary is written under a **recognizable, deterministic name** (a fixed `.tmp-xdev-*.part` prefix/suffix on the destination filesystem); a crash mid-copy can leave such a temp orphaned in a destination directory. Because merge holds the library lock (Section 15.2 — at most one merge per library), any such orphan is necessarily debris from an interrupted prior run, never a concurrent run's live copy, so merge **sweeps stale `.tmp-xdev-*.part` temps from the directories it targets at the start of execute** (and records the count) — keeping the permanent library free of crash litter. The orphan is inert either way (it never bears a final name, so it can never be mistaken for or clobber a real file); the sweep is hygiene, not a safety requirement.
4. **A failed operation is left clean and reported.** If an operation cannot complete (tool failure, unexpected existing target, permission error), the executor leaves the file at its pre-operation state, records the failure, and never records the operation as applied (so resume does not treat it as done). This mirrors geotag Section 29.1a item 4 and prep's blocker handling, and applies equally to merge.
